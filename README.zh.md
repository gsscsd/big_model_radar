...
**需要Cloudflare Workers自托管** — 从 `mcp/` 目录部署自己的实例：
...
之后**`https://big-model-radar-mcp.<your-subdomain>.workers.dev`**即是 MCP 服务器地址。
...
1. 执行 `npm install -g mcporter` 安装 MCP Porter（使用pnpm等包管理器也可）
2. 运行 `mcporter config add big-model-radar --url https://big-model-radar-mcp.<your-subdomain>.workers.dev --transport sse` 添加 MCP 服务器
3. 在 OpenClaw 中启用 mcporter 插件
4. 与你的 OpenClaw 实例交流，尝试获取报告。
